/*
  # User Authentication and Profile System - Fixed Migration
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'user_itineraries' 
    AND rowsecurity = true
  ) THEN
...
DROP POLICY IF EXISTS "Users can view own itineraries" ON user_itineraries;
DROP POLICY IF EXISTS "Users can insert own itineraries" ON user_itineraries;
DROP POLICY IF EXISTS "Users can update own itineraries" ON user_itineraries;
DROP POLICY IF EXISTS "Users can delete own itineraries" ON user_itineraries;

DROP POLICY IF EXISTS "Users can view own journal entries" ON user_journal_entries;
DROP POLICY IF EXISTS "Users can insert own journal entries" ON user_journal_entries;
DROP POLICY IF EXISTS "Users can update own journal entries" ON user_journal_entries;
DROP POLICY IF EXISTS "Users can delete own journal entries" ON user_journal_entries;
